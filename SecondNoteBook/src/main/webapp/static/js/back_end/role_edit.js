...
function doBack(){
    ...
    doClearData();
	//加载列表页面,重新显示查询结果
	var listUrl="tolistRoleUI.do?t="+Math.random(1000);
	$(".container").load(listUrl);
}
/**根据id执行查询获得记录信息*/
function doFindObjectById(id){
	var url="doFindRoleObjectById.do";
	var params={"id":id};
...
function doSetEditFormData(obj){
	$("#typeNameId").val(obj.roleName);
	$("#parentNameId").val(obj.parentRoleName);
	$("#editTypeForm").data("parentRoleId",obj.parentRoleId);
...
function doSaveOrUpdate(){
	...
	//2.发送异步请求提交数据
	var saveUrl="doSaveRoleObject.do";
	var updateUrl="doUpdateRoleObject.do";
...
function doClearData(){
   ...
   $("#editTypeForm").removeData("parentRoleId");
}
function getEditFormData(){
	var params={
	  "roleName":$("#typeNameId").val(),
	  "parentRoleId":$("#editTypeForm").data("parentRoleId")
	};
	return params;
}
/*设置选中的节点*/
function doSetSelectedNode(){
    ...
    $("#editTypeForm").data("parentRoleId",node.id);
    //4.隐藏zTree
    doHideZtree();
}
/**隐藏Ztree*/
